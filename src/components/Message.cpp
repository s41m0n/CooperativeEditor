...
//BASIC MESSAGE
BasicMessage::BasicMessage(Type msgType, unsigned int editorId) : editorId(editorId), msgType(msgType) {
}
...
std::string BasicMessage::toString(int level) {
  return std::string(level, '\t') + "BasicMessage{\n" +
         std::string(level + 1, '\t') + "msgType: " + std::to_string(static_cast<int>(msgType)) + "\n" +
...
//CRDT MESSAGE
CrdtMessage::CrdtMessage(Type msgType, Symbol symbol, unsigned int editorId) : BasicMessage(msgType, editorId),
                                                                               symbol(std::move(symbol)) {
}

CrdtMessage::CrdtMessage(BasicMessage &&msg) : BasicMessage(msg) {
}
...
//REQUEST MESSAGE
RequestMessage::RequestMessage(Type msgType, unsigned int editorId, std::string &filename) : BasicMessage(msgType,
                                                                                                          editorId),
                                                                                             filename(filename) {
}
...
RequestMessage::RequestMessage(BasicMessage &&msg) : BasicMessage(msg) {
}
...
FileContentMessage::FileContentMessage(Type msgType, unsigned int editorId, std::vector<Symbol> &symbols)
        : BasicMessage(msgType, editorId), symbols(symbols) {
}

FileContentMessage::FileContentMessage(BasicMessage &&msg) : BasicMessage(msg) {
}
...
std::string FileContentMessage::toString(int level) {
  ...
  tmp += std::string(level + 1, '\t') + "msgType: " + std::to_string(static_cast<int>(msgType)) + "\n";
  tmp += std::string(level + 1, '\t') + "editorId: " + std::to_string(editorId) + "\n";
